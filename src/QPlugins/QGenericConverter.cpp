                                                                    ...
                                                                    unsigned int acqMode)
{
   ...
   /* ARC
      - at present the ARC support requires an external sampling pattern file
      - this does not recognize autocalibration data that is a dimension of 1 in ky or kz as autocalibration data,
        as that would truely be insufficient
      - this code could be refactored into a separate functional module and be reused for 2D and 3D
      - refactoring could also remove duplicate code for ky and kz analysis
   */
   if(lxData->IsArc()) {
      ...
      if(!std::experimental::filesystem::exists(spfilename)) {
         std::string exception_msg = "sampling file path " + spfilename + " does not exist !";
         throw std::runtime_error(exception_msg.c_str());
      }

      // use Orchestra to read the sampling pattern file
      ...
      if (!samplingPattern->IsValid()) {
         ...
         std::string exception_msg = "No valid sampling pattern loaded for ID: " + std::to_string(patternID);
         throw std::runtime_error(exception_msg.c_str());
      } else {
            ...
            */
            ...
            for (size_t n = 0; n<d.size()-1; n++) {
               if(d[n+1] == 1) {
                  ...
                  throw std::runtime_error("Sampling pattern is not regular ARC sampling");
                  break;
               } else if (n == d.size()-2) {
                  throw std::runtime_error("No ARC autocalibration data found !");
                  break;
               }
            }
      ...
      if (thisPacket->Control().Opcode() != GERecon::Acquisition::ScanControlOpcode)
      {
         ...
         if(static_cast<uint8_t>(packetContents.opcode) != GERecon::Acquisition::ViewCopyOpcode) {
            ...
            else
            {
               ...
               idx.kspace_encode_step_1   = viewID - 1; // TW: viewID of 0 is a baseline frame....
               ...
               // TODO: There could be a little more effort here
               acq.available_channels()   = nChannels;
               acq.discard_pre()          = 0;
               acq.discard_post()         = 0;
               acq.center_sample()        = frame_size/2;
               ...
               */
               ...
               if(dataIndex > expectedFrames-1) {
                 throw std::runtime_error("More data frames received than expected."); 
               }
   ...
   if(!lastMeasFlagSet)
      std::cerr << "WARNING WARNING: Last expected dataframe was never received! Data maybe corrupt!" << std::endl;
   return acqs;
}
